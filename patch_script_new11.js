...
const search = `        // Inject dynamic data before PDF snapshot
...
const replace = `        // Inject dynamic data before PDF snapshot
        updateCalculatorTotal();

        // 1. Clone the Content directly attached to body
        // DO NOT clone a display: none element. Instead, temporarily unhide it!
        ...
        setTimeout(() => {
            // 2. Clone the Content NOW since it is rendered
            const clonedContent = pdfExportArea.cloneNode(true);
            ...
            tempContainer.style.position = 'absolute';
            tempContainer.style.top = '0';
            tempContainer.style.left = '-9999px'; // this causes blank in playwright but let's see. user said left: -9999px
            tempContainer.style.width = '210mm';
